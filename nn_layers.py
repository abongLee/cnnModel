    ...
    with tf.name_scope(layer_name):
        ...
        if use_batch_norm:
            with tf.name_scope('batch_norm'):
                batch_norm = batch_norm_fc(preactivate, output_dim, phase_train=is_training,
...
def conv_layer(input_tensor, kernel_size_x, kernel_size_y,
               input_feat_maps, output_feat_maps, stride, layer_name, act=tf.nn.relu, is_training=True,
               use_batch_norm=True):
    ...
    with tf.name_scope(layer_name):
        ...
        if use_batch_norm:
            with tf.name_scope('batch_norm'):
                batch_norm = batch_norm_conv(preactivate, output_feat_maps, phase_train=is_training,
                                             scope=layer_name + '_batch_norm')
                tf.summary.histogram(layer_name + '/batch_norm', batch_norm)
        else:
            batch_norm = preactivate
...
def batch_norm_fc(x, n_out, phase_train, scope='bn'):
    """
    Batch normalization on fully-connected maps.
    Args:
        x:           Tensor, 2D BD input features
        n_out:       integer, features numbers of input features
        phase_train: boolean tf.Varialbe, true indicates training phase
        scope:       string, variable scope
    Return:
        normed:      batch-normalized maps
